...
import utility
from model.difnet import DIFNet
...
def train(gpu, args):
    ...
    # Network
    if args.model == 'DIFNet':
        net = DIFNet(args)
    else:
        raise TypeError(args.model, ['DIFNet',])
    net.cuda(gpu)

    # # Print model summary and FLOPs
    # dummy_input_rgb = torch.randn(1, 3, 240, 1216).cuda()
    # dummy_input_dep = torch.randn(1, 1, 240, 1216).cuda()
    #
    # macs, params = profile(net, inputs=(dummy_input_rgb, dummy_input_dep))
    # print(f"FLOPs: {macs / 1e9:.2f} G")
    # print(f"Total parameters: {params / 1e6:.2f} M")
    # print('\n' + '=' * 40 + '\n')
    #
    # summary(net, input_size=[(1, 3, 240, 1216), (1, 1, 240, 1216)],
    #         device='cuda',S
    #         verbose=True)
    #
    # import deepspeed.profiling.flops_profiler as flops_profiler
    # prof = flops_profiler.FlopsProfiler(net)
    # prof.start_profile()
    # # 运行一次前向传播
    # _ = net(dummy_input_rgb, dummy_input_dep)
...
def test(args):
    ...
    # # Print model summary and FLOPs
    # dummy_input_rgb = torch.randn(1, 3, 228, 304).cuda()
    # dummy_input_dep = torch.randn(1, 1, 228, 304).cuda()
    #
    # macs, params = profile(net, inputs=(dummy_input_rgb, dummy_input_dep))
    # print(f"FLOPs: {macs / 1e9:.2f} G")
    # print(f"Total parameters: {params / 1e6:.2f} M")
    # print('\n' + '=' * 40 + '\n')
    #
    # summary(net, input_size=[(1, 3, 228, 304), (1, 1, 228, 304)],
    #         device='cuda',
    #         verbose=True)
    #
    # import deepspeed.profiling.flops_profiler as flops_profiler

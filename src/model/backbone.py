...
class Backbone(nn.Module):
    def __init__(self, args, mode='rgbd', image_size=224):
        ...
        if mode == 'rgbd':
            ...
            self.conv1_dep = conv_bn_relu(1, 16, kernel=3, stride=1, padding=1,
                                          bn=False)
            self.conv1 = Earlyfusion(48, 16, 64)
        elif mode == 'rgb':
    ...
    def forward(self, rgb=None, depth=None):
        ...
        if self.mode == 'rgbd':
            ...
            fe1 = self.conv1(fe1_rgb, fe1_dep)
        elif self.mode == 'rgb':
            fe1 = self.conv(rgb)
        elif self.mode == 'd':
            fe1 = self.conv(depth)
        ...
        fe2, fe3, fe4, fe5, fe6, fe7 = self.mamba(fe1)
